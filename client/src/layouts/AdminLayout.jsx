...
export function AdminLayout({ children, activeTab, onTabChange, tabs = [] }) {
  ...
  return (
    <div className="min-h-screen bg-pale flex flex-col">
      {/* Top header bar */}
      <header className="bg-surface border-b border-gold-muted/30 shadow-card z-20 relative">
        <div className="px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xl" aria-hidden="true">🏨</span>
            <span className="font-display font-semibold text-rough">Habesha Grand</span>
            <span className="text-xs text-gold-muted hidden sm:block">/ Admin</span>
          </div>
      ...
      {tabs.length > 0 && (
        <nav
          className="md:hidden fixed bottom-0 inset-x-0 bg-rough border-t border-rough-light/20 z-30 flex overflow-x-auto"
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isActive ? 'text-gold-hover' : 'text-pale/50 hover:text-pale'
                ].join(' ')}
              >
                {Icon && <Icon size={18} strokeWidth={isActive ? 2 : 1.5} aria-hidden="true" />}
